...
// Version: 5.0.0 - WSOEx Timeout Patch
#include <windows.h>
...
typedef BOOL    (WINAPI *tGetFileVersionInfoByHandle)(DWORD, LPCWSTR, DWORD, DWORD, LPVOID); // Note: This is not a standard export but was in the original list.
...
BOOL LoadOriginalVersionDll() {
    ...
    oGetFileVersionInfoByHandle = (tGetFileVersionInfoByHandle)GetProcAddress(g_hRealVersionDll, "GetFileVersionInfoByHandle"); // Keep if it was intended, though non-standard.
    ...
    oGetFileVersionInfoSizeExA  = (tGetFileVersionInfoSizeExA)GetProcAddress(g_hRealVersionDll, "GetFileVersionInfoSizeExA");
    oGetFileVersionInfoSizeExW  = (tGetFileVersionInfoSizeExW)GetProcAddress(g_hRealVersionDll, "GetFileVersionInfoSizeExW");
    ...
    if (!oGetFileVersionInfoSizeW) { // Example check
        FileLog("Error: Failed to get one or more GetProcAddress for version.dll functions.");
...
BOOL InitializeHooks() {
    FileLog("InitializeHooks: Starting initialization (v5.0.0).");

    if (!LoadOriginalVersionDll()) {
        return FALSE; // Error logged in LoadOriginalVersionDll
    }
    FileLog("InitializeHooks: Real version.dll functions mapped.");
    ...
    if (!g_gameModuleBase) {
        FileLog("InitializeHooks: CRITICAL Error! Game module base not found.");
        return FALSE;
    }
    char buffer[128];
    ...
    if (MH_Initialize() != MH_OK) {
        FileLog("InitializeHooks: MH_Initialize failed.");
        return FALSE;
    }
    FileLog("InitializeHooks: MinHook initialized.");

    LPVOID pTargetWSOEx = GetProcAddress(GetModuleHandleW(L"KernelBase.dll"), "WaitForSingleObjectEx");
    if (!pTargetWSOEx) { // Fallback for older systems if KernelBase.dll doesn't have it (unlikely for modern Windows)
        pTargetWSOEx = GetProcAddress(GetModuleHandleW(L"kernel32.dll"), "WaitForSingleObjectEx");
    }
    if (!pTargetWSOEx) {
        FileLog("InitializeHooks: Failed to get address of WaitForSingleObjectEx.");
        MH_Uninitialize();
        return FALSE;
    }

    if (MH_CreateHook(pTargetWSOEx, &DetourWaitForSingleObjectEx_ApplyFix, (LPVOID*)&pfnOriginalWaitForSingleObjectEx) != MH_OK) {
        FileLog("InitializeHooks: MH_CreateHook for WaitForSingleObjectEx failed.");
        MH_Uninitialize();
        return FALSE;
    }
    if (MH_EnableHook(pTargetWSOEx) != MH_OK) {
        FileLog("InitializeHooks: MH_EnableHook for WaitForSingleObjectEx failed.");
        MH_RemoveHook(pTargetWSOEx); // Attempt to clean up partial hook
        MH_Uninitialize();
        return FALSE;
    }
    FileLog("InitializeHooks: WaitForSingleObjectEx hook enabled and patch active.");
    FileLog("InitializeHooks: Initialization successful.");
    return TRUE;
}
...
void UninitializeHooks() {
    ...
    LPVOID pTargetWSOEx = NULL;
    if (pfnOriginalWaitForSingleObjectEx) { // Only attempt to unhook if it was successfully hooked
        pTargetWSOEx = GetProcAddress(GetModuleHandleW(L"KernelBase.dll"), "WaitForSingleObjectEx");
        if (!pTargetWSOEx) pTargetWSOEx = GetProcAddress(GetModuleHandleW(L"kernel32.dll"), "WaitForSingleObjectEx");
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    switch (ul_reason_for_call) {
        case DLL_PROCESS_ATTACH:
            ...
            if (!InitializeHooks()) {
                FileLog("DllMain: InitializeHooks FAILED during DLL_PROCESS_ATTACH.");
...
BOOL WINAPI GetFileVersionInfoByHandle(DWORD dwFlags, LPCWSTR lpszFileName, DWORD dwHandle, DWORD dwLen, LPVOID lpData) {
...
DWORD WINAPI GetFileVersionInfoSizeA(LPCSTR lptstrFilename, LPDWORD lpdwHandle) {
    ...
    if (lpdwHandle) *lpdwHandle = 0; SetLastError(ERROR_PROC_NOT_FOUND); return 0;
}
...
DWORD WINAPI VerFindFileA(DWORD uFlags, LPSTR szFileName, LPSTR szWinDir, LPSTR szAppDir, LPSTR szCurDir, PUINT lpuCurDirLen, LPSTR szDestDir, PUINT lpuDestDirLen) {
    if (oVerFindFileA) return oVerFindFileA(uFlags, szFileName, szWinDir, szAppDir, szCurDir, lpuCurDirLen, szDestDir, lpuDestDirLen);
...
DWORD WINAPI VerFindFileW(DWORD uFlags, LPWSTR szFileName, LPWSTR szWinDir, LPWSTR szAppDir, LPWSTR szCurDir, PUINT lpuCurDirLen, LPWSTR szDestDir, PUINT lpuDestDirLen) {
    if (oVerFindFileW) return oVerFindFileW(uFlags, szFileName, szWinDir, szAppDir, szCurDir, lpuCurDirLen, szDestDir, lpuDestDirLen);
    SetLastError(ERROR_PROC_NOT_FOUND); return 0;
}
DWORD WINAPI VerInstallFileA(DWORD uFlags, LPSTR szSrcFileName, LPSTR szDestFileName, LPSTR szSrcDir, LPSTR szDestDir, LPSTR szCurDir, LPSTR szTmpFile, PUINT lpuTmpFileLen) {
    if (oVerInstallFileA) return oVerInstallFileA(uFlags, szSrcFileName, szDestFileName, szSrcDir, szDestDir, szCurDir, szTmpFile, lpuTmpFileLen);
    SetLastError(ERROR_PROC_NOT_FOUND); return 0;
}
DWORD WINAPI VerInstallFileW(DWORD uFlags, LPWSTR szSrcFileName, LPWSTR szDestFileName, LPWSTR szSrcDir, LPWSTR szDestDir, LPWSTR szCurDir, LPWSTR szTmpFile, PUINT lpuTmpFileLen) {
    if (oVerInstallFileW) return oVerInstallFileW(uFlags, szSrcFileName, szDestFileName, szSrcDir, szDestDir, szCurDir, szTmpFile, lpuTmpFileLen);
    SetLastError(ERROR_PROC_NOT_FOUND); return 0;
}
DWORD WINAPI VerLanguageNameA(DWORD wLang, LPSTR szLang, DWORD nSize) {
    if (oVerLanguageNameA) return oVerLanguageNameA(wLang, szLang, nSize);
    if (nSize > 0 && szLang) szLang[0] = '\0'; SetLastError(ERROR_PROC_NOT_FOUND); return 0;
}
DWORD WINAPI VerLanguageNameW(DWORD wLang, LPWSTR szLang, DWORD nSize) {
    if (oVerLanguageNameW) return oVerLanguageNameW(wLang, szLang, nSize);
    if (nSize > 0 && szLang) szLang[0] = L'\0'; SetLastError(ERROR_PROC_NOT_FOUND); return 0;
}
BOOL WINAPI VerQueryValueA(LPCVOID pBlock, LPCSTR lpSubBlock, LPVOID *lplpBuffer, PUINT puLen) {
    if (oVerQueryValueA) return oVerQueryValueA(pBlock, lpSubBlock, lplpBuffer, puLen);
    if (lplpBuffer) *lplpBuffer = NULL; if (puLen) *puLen = 0; SetLastError(ERROR_PROC_NOT_FOUND); return FALSE;
}
